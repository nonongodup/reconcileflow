...
export function stripePrice(plan: Exclude<PlanKey,"free">, interval: BillingInterval) {
  const key = `STRIPE_${plan.toUpperCase()}_${interval === "month" ? "MONTHLY" : "ANNUAL"}_PRICE_ID`;
  const price = process.env[key];
  if (!price) throw new Error("BILLING_CONFIGURATION");
  return price;
}

export function planForPrice(priceId: string): { plan: Exclude<PlanKey,"free">; interval: BillingInterval } | null {
  const entries: Array<[Exclude<PlanKey,"free">,BillingInterval,string|undefined]> = [
