...
export async function POST(request: Request) {
  ...
  if (
    ...
    const price = subscription.items.data[0]?.price.id,
      mapped = price ? planForPrice(price) : null;
    const customerId =
    ...
    const target = userId
      ? await sql`SELECT id FROM rf_users WHERE id=${userId}`
      : await sql`SELECT user_id AS id FROM rf_subscriptions WHERE stripe_customer_id=${customerId}`;
    if (target[0])
      await sql`INSERT INTO rf_subscriptions (user_id,stripe_customer_id,stripe_subscription_id,plan,billing_interval,status,current_period_start,current_period_end,cancel_at_period_end,updated_at)
      VALUES (${target[0].id},${customerId},${subscription.id},${
        mapped?.plan || "free"
      },${mapped?.interval || null},${subscription.status},${new Date(
        subscription.items.data[0]?.current_period_start * 1000
      ).toISOString()},${new Date(
        subscription.items.data[0]?.current_period_end * 1000
      ).toISOString()},${subscription.cancel_at_period_end},now())
