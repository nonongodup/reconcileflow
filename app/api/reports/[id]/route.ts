...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const user = await requireApiUser(request), { id } = await context.params, sql = getSql();
    const rows = await sql`SELECT id,report_key,report_expires_at,report_deleted_at FROM rf_runs WHERE id=${id} AND user_id=${user.id} AND status='completed'`;
    ...
    if (!run.report_expires_at || new Date(run.report_expires_at).getTime() <= Date.now()) {
      await deleteTrackedObject(bucket, run.report_key, requestId); await sql`UPDATE rf_runs SET report_deleted_at=now() WHERE id=${id} AND user_id=${user.id}`;
    ...
    await sql`UPDATE rf_runs SET report_deleted_at=now() WHERE id=${id} AND user_id=${user.id}`;
