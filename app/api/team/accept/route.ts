...
export async function POST(request: Request) {
  try {
    ...
    const seats =
      await sql`SELECT count(*)::int AS count FROM rf_workspace_members WHERE workspace_id=${invitation.workspace_id}`;
    if (Number(seats[0]?.count || 0) >= 10)
