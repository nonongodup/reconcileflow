...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const owned = await sql`SELECT id FROM rf_runs WHERE id=${id} AND user_id=${user.id}`; if (!owned[0]) throw new SafeError("NOT_FOUND", 404);
    ...
    await sql`DELETE FROM rf_runs WHERE id=${id} AND user_id=${user.id}`;
