...
import { assertFormat, getPlanAccess, hasAdvancedRules, reserveReconciliation } from "../plans";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!access.entitlements.advancedRules && hasAdvancedRules(rules as Record<string, unknown>)) throw new SafeError("PLAN_FEATURE", 403);
    await reserveReconciliation(access.scopeId, access.entitlements.runsPerMonth);
    ...
    await sql`INSERT INTO rf_runs (id,user_id,workspace_id,name,status,source_name,target_name,source_format,target_format,mappings,rules)
      VALUES (${runId},${user.id},${access.workspaceId},${String(form.get("name") || "File reconciliation")},'processing',${sourceFile.name},${targetFile.name},${sourceFile.name.toLowerCase().endsWith(".xlsx") ? "XLSX" : "CSV"},${targetFile.name.toLowerCase().endsWith(".xlsx") ? "XLSX" : "CSV"},${JSON.stringify(mappings)}::jsonb,${JSON.stringify(rules)}::jsonb)`;
    const scans = await Promise.all([
      scanUpload(sourceBytes, sourceFile.name, { requestId, userId: user.id, runId }),
      scanUpload(targetBytes, targetFile.name, { requestId, userId: user.id, runId }),
    ]);
    await audit("malware_scan_completed", requestId, user.id, runId, "malware_scan", scans.every((item) => item.status === "clean") ? "CLEAN" : "SCAN_NOT_ENFORCED");
    ...
    const response = await withTemporaryUploads(bucket, [
      ...
      const { source, target, result } = await withTimeout(Promise.resolve(processFiles(
        ...
        mappings, rules, { maxRows: access.entitlements.rowsPerFile, maxColumns: limits.maxColumns, maxWorksheets: limits.maxWorksheets, maxCsvRowLength: limits.maxCsvRowLength },
    ...
    if (safe.code === "MALWARE_DETECTED" && runId) await audit("malware_scan_rejected", requestId, userId, runId, "malware_scan", safe.code).catch(() => undefined);
    if (safe.status >= 500 || safe.code === "MALWARE_DETECTED") {
      await emitSecurityEvent({ event: safe.code === "MALWARE_DETECTED" ? "upload_rejected" : "server_error", requestId, userId, runId, stage: "reconcile", safeCode: safe.code, severity: safe.code === "MALWARE_DETECTED" ? "critical" : "high" });
    }
